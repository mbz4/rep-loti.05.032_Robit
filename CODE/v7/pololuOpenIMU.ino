...
void runOpenIMU() {
  ...
  if (millis() - displayTimer > 30) {
    ...
    Serial.print("\t YAW:");
    Serial.println(yawInDegrees);
    /*
        //if (yawInDegrees //add conditionals if on sides...
        //ROLL:179.75   PITCH: -0.29  YAW:90.72 //on roof (start on roof)
        //ROLL:17.55   PITCH: -34.77   YAW:227.81 // on wheels
        //ROLL:-177.86   PITCH: -0.67  YAW:89.96 // (start) on wheels
        // ROLL:-68.74   PITCH: -28.36   YAW:205.97 // on right side
        // ROLL:82.00  PITCH: 26.10  YAW:33.60 // on left side
        // ROLL:-75.29  PITCH: 58.45  YAW:235.45 // on back side
        // ROLL:95.99  PITCH: -40.46   YAW:61.09 // on front side
        if (rollInDegrees < 110 && rollInDegrees > 80) { // on the side
          motorsStop();
          Serial.print("\t roll = ");
          Serial.print(rollInDegrees);
          Serial.println("\t Motors off!");
        } else if ((rollInDegrees < -110 && rollInDegrees > -80)) {
          motorsStop();
          Serial.print("\t roll = ");
          Serial.print(rollInDegrees);
          Serial.println("\t Motors off!");
